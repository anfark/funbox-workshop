...
Position moveHorizontal(Position position, int direction, Size bounds) {
  position.x += direction;

  if (position.x < 0) {
    position.x = bounds.w - 1;
  }
...
bool hitsObstacle(const FroggerState& state) {
  return contains(
    state.obstacles,
    state.frog
  );
...
bool reachedGoal(const FroggerState& state) {
  return state.frog.y == BaseGame::bounds.h - 1;
}


// Prüft, ob die verfügbare Zeit abgelaufen ist.
bool timedOut(const FroggerState& state) {
  return state.ticks >= FroggerGame::MAX_TICKS;
}
...
void FroggerUpdate::move(Direction direction) {
  const Position next =
    state.frog + direction;

  if (next.isOutside(bounds)) {
    return;
  }
...
void FroggerUpdate::detectEnd() {
  if (hitsObstacle(state)) {
    event(DidHitObstacle{});
    return;
  }
