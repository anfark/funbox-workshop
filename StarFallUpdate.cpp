...
bool anyStarHitsShip(
  const std::vector<Position>& stars,
  int shipX
) {
  for (
    const auto& star :
...
// Erzeugt eine freie Startposition für einen neuen Stern.
...
// Prüft, ob abhängig von der Sternanzahl ein neuer Stern entstehen soll.
...
// Entfernt einen getroffenen Schuss und Stern und liefert die Trefferposition zurück.
bool removeHit(
